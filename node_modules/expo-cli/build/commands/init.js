...
async function action(projectDir, options) {
  ...
  if (options.template) {
    ...
    let descriptionColumn = Math.max(...FEATURED_TEMPLATES.map(t => t.shortName ? t.shortName.length : 0)) + 2;
    let {
      template
      ...
      nonInteractiveHelp: '--template: argument is required in non-interactive mode. Valid choices are: ' + FEATURED_TEMPLATES.filter(({
        shortName
      }) => shortName).map(template => `'${template.shortName}'`).join(', ') + ' or any custom template (name of npm package).'
    });
  ...
  let projectPath = await _xdl().Exp.extractAndInitializeTemplateApp(templateSpec, _path().default.join(parentDir, dirName || initialConfig.name || initialConfig.expo.slug), packageManager, initialConfig);
...
async function promptForBareConfig(parentDir, dirName, options) {
  ...
  for (let key of Object.keys(values)) {
    (0, _set().default)(config, key, values[key]);
  }
